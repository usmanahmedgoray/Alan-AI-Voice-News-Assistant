    ...
    return (
        <>
            ...
            <div ref={modalRef} id="drawer-bottom-example" className={`w-auto md:w-[33rem] h-[38rem] bg-white border  bottom-0 fixed left-0 right-0 z-40 m-2 p-6 overflow-y-auto transition-transformdark:bg-gray-800 transform-none rounded-md `} tabIndex="-1" aria-labelledby="drawer-bottom-label">
                ...
                <img src={imageUrl || "https://www.niddk.nih.gov/-/media/Images/Components/Default-Social-Media-Images/News-Card.png"} alt="media" className="w-[31rem] hover:shadow-md rounded-lg" />
                {/* Title of the Summarizing Article */}
                <h2 className="mt-6 text-2xl text-justify">{Summary.title}</h2>
                ...
                {ArticleSummary&& <div className="link my-6">
                    ...
                    <a href={Summary.link || Summary.url} target="_blank" rel="noreferrer" className="hover:underline underline-offset-2 hover:text-blue-800">{Summary.link || Summary.url}</a>
                </div>}
            </div>
        </>
    )
